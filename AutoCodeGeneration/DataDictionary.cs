...
namespace AutoCodeGeneration
{
                ...
                foreach (var item in tabes)
                {
                    ...
                    if (records != null)
                    {
                        for (int i = 0; i < records.Rows.Count; i++)
                        {
                            ...
                            dataRecord.DataTable = records.Rows[i]["数据表"] is DBNull ? String.Empty : records.Rows[i]["数据表"].ToString();
    ...
    public class DataRecord
    {
        ...
        {
            if (DataTable.Equals(className) && !IsClassRecord(null)) return true;
            return false;
        }
